...
def init_sentry(
    dsn: Optional[str] = None, environment: str = "development", **kwargs
) -> None:
    """
    Sentry SDK를 초기화합니다.
...
def init_sentry_for_fastapi(
    app: FastAPI, dsn: Optional[str] = None, environment: str = "development"
) -> None:
    """
...
def capture_error(
    exception: Exception, user_id: Optional[str] = None, **extra_context
) -> None:
    """
...
def set_request_context(method: str, url: str, **extra_data) -> None:
    ...
    """
    sentry_sdk.set_context("request", {"method": method, "url": url, **extra_data})
