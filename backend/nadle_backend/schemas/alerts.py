...
class AlertRuleCreate(BaseModel):
    ...
    description: Optional[str] = Field(
        None, description="알림 규칙 설명", max_length=500
    )
    condition: str = Field(
        ..., description="알림 조건 (greater_than, less_than, equals)"
    )
    ...
    channels: List[str] = Field(
        ..., description="알림 채널 목록 (email, discord)", min_items=1
    )
    ...
    def validate_condition(cls, v):
        ...
        if v not in valid_conditions:
            raise ValueError(
                f"유효하지 않은 조건: {v}. 사용 가능한 조건: {valid_conditions}"
            )
        return v
    ...
    def validate_severity(cls, v):
        ...
        if v not in valid_severities:
            raise ValueError(
                f"유효하지 않은 심각도: {v}. 사용 가능한 심각도: {valid_severities}"
            )
        return v
    ...
    def validate_channels(cls, v):
        ...
        for channel in v:
            if channel not in valid_channels:
                raise ValueError(
                    f"유효하지 않은 채널: {channel}. 사용 가능한 채널: {valid_channels}"
                )
        return v
