...
class Settings(BaseSettings):
    ...
    users_collection: str = Field(
        default="users", description="사용자 문서를 저장할 컬렉션 이름"
    )
    posts_collection: str = Field(
        default="posts", description="게시글 문서를 저장할 컬렉션 이름"
    )
    comments_collection: str = Field(
        default="comments", description="댓글 문서를 저장할 컬렉션 이름"
    )
    post_stats_collection: str = Field(
        default="post_stats", description="게시글 통계를 저장할 컬렉션 이름"
    )
    ...
    files_collection: str = Field(
        default="files", description="파일 메타데이터를 저장할 컬렉션 이름"
    )
    stats_collection: str = Field(
        default="stats", description="애플리케이션 통계를 저장할 컬렉션 이름"
    )
    ...
    algorithm: str = Field(default="HS256", description="JWT 서명 알고리즘")
    access_token_expire_minutes: int = Field(
        default=30, gt=0, description="액세스 토큰 만료 시간 (분 단위)"
    )
    refresh_token_expire_days: int = Field(
        default=7, gt=0, description="리프레시 토큰 만료 시간 (일 단위)"
    )
    ...
    # === API 설정 ===
    api_title: str = Field(
        default="Content Management API", description="API 서비스 제목"
    )
    api_version: str = Field(default="1.0.0", description="API 버전 번호")
                ...
                with open(env_file, "r", encoding="utf-8") as f:
                    for line in f:
                        if line.strip().startswith("ENVIRONMENT="):
                            ...
                            print(
                                f"✅ ENVIRONMENT 설정 확인: {env_from_file} (파일: {env_file})"
                            )
                            explicitly_set = True
                            break
    ...
    log_level: Literal["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"] = Field(
        default="INFO", description="애플리케이션 로깅 레벨"
    )

    # Feature Flags
    enable_docs: bool = Field(
        default=True, description="Enable automatic API documentation (Swagger/OpenAPI)"
    )
    enable_cors: bool = Field(
        default=True, description="Enable Cross-Origin Resource Sharing (CORS)"
    )

    # Email Configuration
    smtp_server: str = Field(
        default="smtp.gmail.com", description="SMTP server for sending emails"
    )
    smtp_port: int = Field(
    ...
    from_email: str = Field(
        default="noreply@example.com", description="Email address for sending emails"
    )
    from_name: str = Field(
        default="XAI Community", description="Display name for sender"
    )
    ...
    email_verification_max_attempts: int = Field(
        default=5, gt=0, description="Maximum verification attempts before blocking"
    )
    ...
    redis_db: int = Field(
        default=0, ge=0, le=15, description="Redis 데이터베이스 번호 (0-15)"
    )
    redis_password: Optional[str] = Field(
        default=None, description="Redis 비밀번호 (설정된 경우)"
    )
    ...
    cache_ttl_user: int = Field(
        default=3600, gt=0, description="사용자 정보 캐시 TTL (초 단위)"
    )
    cache_enabled: bool = Field(default=True, description="Redis 캐시 활성화 여부")
    ...
    def use_upstash_redis(self) -> bool:
        ...
        return (
            ...
            and self.upstash_redis_rest_url is not None
    ...
    security_headers_enabled: bool = Field(
        default=True, description="보안 헤더 미들웨어 활성화 여부"
    )
    hsts_max_age: int = Field(
        default=31536000, gt=0, description="HSTS 헤더 max-age 값 (초 단위)"  # 1년
    )
    csp_report_uri: Optional[str] = Field(
        default=None, description="CSP 위반 보고서 수집 URI"
    )

    # === 환경변수 보안 설정 ===
    mask_sensitive_logs: bool = Field(
        default=True, description="로그에서 민감한 정보 마스킹 여부"
    )
    environment_security_check: bool = Field(
        default=True, description="시작시 환경변수 보안 검증 여부"
    )
    ...
    hetrixtools_api_token: Optional[str] = Field(
        default=None, description="HetrixTools API 토큰"
    )

    # === 인프라 모니터링 설정 ===
    # Google Cloud Run
    gcp_project_id: Optional[str] = Field(
        default=None, description="Google Cloud 프로젝트 ID - Cloud Run 모니터링용"
    )
    ...
    gcp_service_name: Optional[str] = Field(
        default=None, description="Google Cloud Run 서비스 이름"
    )
    gcp_region: Optional[str] = Field(
        default=None, description="Google Cloud Run 서비스 리전"
    )

    # Vercel
    vercel_api_token: Optional[str] = Field(default=None, description="Vercel API 토큰")
    vercel_team_id: Optional[str] = Field(default=None, description="Vercel 팀 ID")
    ...
    upstash_email: Optional[str] = Field(
        default=None, description="Upstash 계정 이메일 - API 인증용"
    )
    upstash_api_key: Optional[str] = Field(
        default=None, description="Upstash Dashboard API 키"
    )
    ...
    def validate_secret_key(cls, v: str) -> str:
        ...
        if any(pattern in v.lower() for pattern in unsafe_patterns):
            ...
            if environment == "production":
                raise ValueError(
                    "프로덕션에서 안전하지 않은 비밀키를 사용할 수 없습니다"
                )
            else:
                print(
                    f"⚠️  경고: 안전하지 않은 비밀키가 감지되었습니다. 프로덕션에서는 안전한 비밀키를 사용하세요."
                )
    ...
    def validate_mongodb_url(cls, v: str) -> str:
        ...
        if not v.startswith(("mongodb://", "mongodb+srv://")):
            raise ValueError(
                "MongoDB URL은 mongodb:// 또는 mongodb+srv://로 시작해야 합니다"
            )
        return v
        ...
        if self.allowed_origins is None:
            if environment == "production":
                ...
                if allowed_origins_env:
                    ...
                    print(
                        "❌ 오류: 프로덕션에서 ALLOWED_ORIGINS 환경변수가 설정되지 않았습니다!"
                    )
                    raise ValueError(
                        "프로덕션에서 ALLOWED_ORIGINS 환경변수가 필수입니다"
                    )
            else:
                # 개발환경: 안전한 로컬 기본값
                self.allowed_origins = [
    ...
    def _perform_security_check(self):
        """환경변수 보안 검증 수행"""
        try:
            from nadle_backend.utils.security import (
    ...
    def validate_production_settings(self):
        ...
        if not self.allowed_origins:
            ...
            errors.append(
                "ALLOWED_ORIGINS에 localhost URL이 포함되어 있음 (프로덕션 부적절)"
            )
