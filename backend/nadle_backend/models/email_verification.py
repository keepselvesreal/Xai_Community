...
class EmailVerificationData(BaseModel):
    ...
    last_attempt_at: Optional[datetime] = Field(
        default=None, description="Last verification attempt time"
    )
    ...
    def create_verification(
        cls, email: str, code: str, expire_minutes: int = None, created_ip: str = None
...
class EmailVerificationCodeResponse(BaseModel):
    ...
    can_proceed: bool = Field(
        ..., description="Whether user can proceed to registration"
    )
    message: str = Field(..., description="User-friendly message")
