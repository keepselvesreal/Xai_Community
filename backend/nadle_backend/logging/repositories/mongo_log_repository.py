...
class MongoLogRepository(LogRepositoryInterface):
    ...
    async def setup_indexes(self) -> None:
        ...
        try:
            ...
            indexes = [
                ...
                IndexModel([
                    ("context.user_id", ASCENDING),
                    ("timestamp", DESCENDING)
                ], name="user_timestamp", sparse=True),
                
                IndexModel([
                    ("context.endpoint", ASCENDING),
                    ("level", ASCENDING),
                    ("timestamp", DESCENDING)
                ], name="endpoint_level_timestamp", sparse=True),
                ...
                ], name="error_grouping", sparse=True),
                ...
                IndexModel([
                    ...
                    ("timestamp", DESCENDING)
                ], name="performance_analysis", sparse=True),
    ...
    async def get_time_series(
        ...
        try:
            ...
            pipeline = [
                ...
                {"$group": {
                    "_id": "$_id.timestamp",
                    "error": {"$sum": {"$cond": [{"$eq": ["$_id.level", "ERROR"]}, "$count", 0]}},
                    "warn": {"$sum": {"$cond": [{"$eq": ["$_id.level", "WARN"]}, "$count", 0]}},
                    "info": {"$sum": {"$cond": [{"$eq": ["$_id.level", "INFO"]}, "$count", 0]}},
                    "debug": {"$sum": {"$cond": [{"$eq": ["$_id.level", "DEBUG"]}, "$count", 0]}}
                }},
                {"$sort": {"_id": 1}}
            ]
    ...
    def _build_query(self, filter_obj: LogFilter) -> Dict[str, Any]:
        ...
        if filter_obj.services:
            query["service"] = {"$in": [service.value for service in filter_obj.services]}
