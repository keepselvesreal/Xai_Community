...
class ExternalLogCollectorService:
    ...
    async def collect_all_logs(self, hours: int = 1) -> Dict[str, Any]:
        ...
        logger.info(
            f"Starting external log collection for {hours} hours from {len(self.adapters)} adapters"
        )
        ...
        # Process results
        for i, (adapter_name, result) in enumerate(
            zip(self.adapters.keys(), adapter_results)
        ):
    ...
    async def collect_from_service(
        ...
        try:
            ...
            logger.info(
                f"Successfully collected {result['logs_collected']} logs from {service_name}"
            )
            return result
    ...
    async def _collect_from_adapter(
        self, adapter_name: str, adapter: ExternalLogAdapterInterface, hours: int
    ) -> Dict[str, Any]:
        """
        ...
        async with self._collection_semaphore:
            ...
            try:
                ...
                logs = await asyncio.wait_for(
                    adapter.collect_logs(hours), timeout=self.collection_timeout
                )
                ...
                raise AdapterError(
                    adapter_name, f"Collection timeout after {self.collection_timeout}s"
                )
            except Exception as e:
                raise AdapterError(adapter_name, f"Collection failed: {str(e)}")

    async def _test_adapter_connection(
        self, adapter_name: str, adapter: ExternalLogAdapterInterface
    ) -> bool:
        """
    ...
    async def _log_collection_error(
        self, adapter_name: str, error_message: str
    ) -> None:
        """
    ...
    async def _log_collection_completion(self, results: Dict[str, Any]) -> None:
        ...
        try:
            ...
            log_entry = LogEntry(
                ...
                context=LogContext(
                    infrastructure="external_collector",
                    response_time=results.get("duration_seconds", 0)
                    * 1000,  # Convert to ms
                ),
                metadata={
