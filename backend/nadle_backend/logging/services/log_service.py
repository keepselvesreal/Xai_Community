...
class LogService:
    ...
    async def get_stats(self, hours: int = 24) -> LogStats:
        ...
        try:
            ...
            if self.cache_service and cache_key:
                await self._cache_result(cache_key, stats.model_dump(), ttl=60)  # 1 minute
    ...
    async def get_dashboard_data(self, hours: int = 24) -> LogDashboardResponse:
        ...
        try:
            ...
            errors_task = self.log_repository.get_error_groupings(start_time, end_time, 5)
            time_series_task = self.log_repository.get_time_series(start_time, end_time, 60)
            endpoints_task = self.log_repository.get_top_endpoints(start_time, end_time, 10)
            ...
            performance_summary = self._calculate_performance_summary(stats, top_endpoints)
            ...
            if self.cache_service and cache_key:
                await self._cache_result(cache_key, dashboard_data.model_dump(), ttl=120)  # 2 minutes
    ...
    async def cleanup_old_logs(self, retention_days: int = 30) -> Dict[str, int]:
        ...
        try:
            ...
            retention_policy = {
                ...
                LogLevel.WARN.value: cutoff_date + timedelta(days=7),  # Keep warnings a bit longer
                LogLevel.INFO.value: cutoff_date + timedelta(days=23),  # Standard retention
                LogLevel.DEBUG.value: cutoff_date + timedelta(days=29),  # Keep debug logs shortest
            }
            
            cleanup_results = {}
            for level, level_cutoff in retention_policy.items():
                deleted_count = await self.log_repository.delete_logs_before(level_cutoff, level)
    ...
    async def _cache_result(self, cache_key: str, data: Dict[str, Any], ttl: Optional[int] = None) -> None:
        """
        Cache result data.
    ...
    def _calculate_performance_summary(
        ...
        for service, count in stats.service_stats.items():
            ...
            service_endpoints = [ep for ep in top_endpoints if service.lower() in ep.endpoint.lower()]
            
            if service_endpoints:
                avg_response_time = sum(ep.avg_response_time for ep in service_endpoints) / len(service_endpoints)
                max_response_time = max(ep.avg_response_time for ep in service_endpoints)
