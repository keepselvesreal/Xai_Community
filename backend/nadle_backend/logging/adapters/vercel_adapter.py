...
class VercelLogAdapter(ExternalLogAdapterInterface):
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                async with session.get(
                    f"{self.base_url}/v9/projects", headers=self.headers
                ) as response:
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                for deployment in deployments:
                    deployment_logs = await self._get_deployment_logs(
                        session, deployment
                    )
    ...
    async def _get_deployments(
        self, session: aiohttp.ClientSession, hours: int
    ) -> List[Dict[str, Any]]:
        """
        ...
        async with session.get(url, headers=self.headers, params=params) as response:
            if response.status != 200:
                raise AdapterError(
                    "vercel", f"Failed to get deployments: {response.status}"
                )
    ...
    async def _get_deployment_logs(
        self, session: aiohttp.ClientSession, deployment: Dict[str, Any]
    ) -> List[LogEntry]:
        """
        ...
        try:
            ...
            async with session.get(url, headers=self.headers) as response:
                if response.status == 200:
                    ...
                    logger.warning(
                        f"Failed to get events for deployment {deployment_id}: {response.status}"
                    )
    ...
    def _deployment_to_log_entry(self, deployment: Dict[str, Any]) -> LogEntry:
        ...
        context = LogContext(
            ...
            region=(
                deployment.get("regions", ["unknown"])[0]
    ...
    def _event_to_log_entry(
        self, event: Dict[str, Any], deployment: Dict[str, Any]
    ) -> LogEntry:
        """
        ...
        return {
            "deployments": [
                ...
                },
                {
                    ...
                    "target": None,
                    "created": int(
                        (datetime.utcnow() - timedelta(minutes=45)).timestamp() * 1000
                    ),
                    "creator": "mock-user",
                    "meta": {
                        "githubCommitAuthorName": "nadle",
    ...
    def _get_mock_deployment_events(
        self, deployment_id: str, event_count: int = 5
    ) -> List[Dict[str, Any]]:
