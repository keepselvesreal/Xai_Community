...
class CloudRunLogAdapter(ExternalLogAdapterInterface):
    ...
    def _cloud_log_to_log_entry(self, cloud_log: Dict[str, Any]) -> LogEntry:
        ...
        context = LogContext(
            ...
            instance_id=cloud_log.get("labels", {}).get("instanceId", "")[
                :16
            ],  # Truncate for readability
            endpoint=(
                http_request.get("request_url", "").split("?")[0]
                if http_request.get("request_url")
                else None
            ),
        ...
        metadata = LogMetadata(
            cloud_trace_id=(
                cloud_log.get("trace", "").split("/")[-1]
                if cloud_log.get("trace")
                else None
            ),
            tags=[
                "cloud_run",
        ...
        if source_location and level == LogLevel.ERROR:
            stack_trace = (
                f"File: {source_location.get('file', 'unknown')}\n"
            ...
            LogEntry(
                ...
                metadata=LogMetadata(
                    ...
                    custom={
                        ...
                        "service_name": self._service_name_filter
                        or "xai-community-backend",
                    },
                ),
            )
        )
