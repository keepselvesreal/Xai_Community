...
class UpstashLogAdapter(ExternalLogAdapterInterface):
    ...
    def __init__(
        self, rest_url: str, rest_token: str, use_mock: bool = False, timeout: int = 30
    ):
        """
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                async with session.get(
                    f"{self.rest_url}/ping", headers=self.headers
                ) as response:
    ...
    async def _collect_ping_metric(self, session: aiohttp.ClientSession) -> LogEntry:
        ...
        return LogEntry(
            ...
            source=LogSource.EXTERNAL,
            message=message,
            context=LogContext(infrastructure="upstash", response_time=response_time),
            metadata=LogMetadata(
    ...
    async def _collect_info_metrics(
        ...
        """
        async with session.get(
            f"{self.rest_url}/info", headers=self.headers
        ) as response:
            if response.status == 200:
                data = await response.json()
    ...
    async def _collect_dbsize_metric(self, session: aiohttp.ClientSession) -> LogEntry:
        ...
        """
        async with session.get(
            f"{self.rest_url}/dbsize", headers=self.headers
        ) as response:
            timestamp = datetime.utcnow()
    ...
    async def _collect_key_metrics(
        ...
        for pattern in patterns:
            async with session.get(
                f"{self.rest_url}/keys/{pattern}", headers=self.headers
            ) as response:
                timestamp = datetime.utcnow()
            ...
            logs.append(
                LogEntry(
                    ...
                    metadata=LogMetadata(
                        ...
                        memory_usage=self._parse_memory_bytes(
                            memory_section.get("used_memory", "0")
                        ),
                        custom={
                            "used_memory_human": used_memory,
                            "maxmemory_human": max_memory,
                            "memory_policy": memory_section.get(
                ...
                LogEntry(
                    ...
                    metadata=LogMetadata(
                        ...
                        custom={
                            ...
                            "keyspace_hits": int(
                                stats_section.get("keyspace_hits", "0")
                            ),
                            "keyspace_misses": int(
                                stats_section.get("keyspace_misses", "0")
                            ),
                        },
                    ),
                )
