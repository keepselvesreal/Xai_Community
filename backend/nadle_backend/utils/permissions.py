...
class PermissionChecker:
    ...
    def check_resource_ownership(
        self, user: Optional[Any], resource: Optional[Any]
    ) -> bool:
    ...
    def check_admin_permission(self, user: Optional[Any]) -> bool:
        ...
        try:
            ...
            return (
                is_admin is True
            )  # Explicitly check for True to avoid MagicMock issues
        except (AttributeError, TypeError):
            return False
    ...
    def check_post_permission(
        self, user: Optional[Any], post: Optional[Any], permission: ResourcePermission
    ) -> bool:
        ...
        try:
            ...
            print(
                f"Permission check: user_id={user_id_str}, post_author_id={post_author_id_str}"
            )
            print(
                f"Type of user_id: {type(user_id)}, Type of post_author_id: {type(post_author_id)}"
            )
    ...
    def can_user_access_resource(
        ...
        try:
            ...
            elif hasattr(resource, "content") and hasattr(
                resource, "post_id"
...
def check_post_permission(user, post, action: str) -> bool:
    ...
    """
    if action == "read":
        return permission_checker.check_post_permission(
            user, post, ResourcePermission.READ
        )
    elif action in ["update", "write"]:
        return permission_checker.check_post_permission(
            user, post, ResourcePermission.WRITE
        )
    elif action == "delete":
        return permission_checker.check_post_permission(
            user, post, ResourcePermission.DELETE
        )
    else:
        return False
