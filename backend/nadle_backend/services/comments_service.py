...
class CommentsService:
    ...
    async def create_comment(
        self, post_slug: str, comment_data: CommentCreate, current_user: User
    ) -> CommentDetail:
        ...
        comment_data.metadata["route_path"] = self._generate_route_path(
            normalized_type, post.slug
        )
    ...
    async def get_comments_with_user_data(
        ...
        comments_with_replies, total = (
            await self.comment_repo.get_comments_with_replies(
                post_id=str(post.id),
                page=page,
                page_size=page_size,
                status="active",
        ...
        for i, item in enumerate(comments_with_replies):
            ...
            print(
                f"🔍 [DEBUG] 댓글 {i+1}: id={comment.id}, content={comment.content[:30]}..., replies={len(replies)}"
            )
            for j, reply_item in enumerate(replies):
                reply_comment = reply_item["comment"]
                print(
                    f"    답글 {j+1}: id={reply_comment.id}, content={reply_comment.content[:30]}..."
                )
    ...
    async def create_reply(
        ...
        print(
            f"🔍 [DEBUG] 답글 공개/비공개 상속: parent_id={parent_comment_id}, parent_is_public={parent_is_public}"
        )
        ...
        # Create reply
        print(
            f"🔍 [DEBUG] 답글 생성: parent_comment_id={parent_comment_id}, content={comment_data.content[:30]}..."
        )
        ...
        print(
            f"🔍 [DEBUG] 답글 생성 완료: reply_id={reply.id}, parent_comment_id={reply.parent_comment_id}"
        )
    ...
    async def update_comment_with_permission(
        self, comment_id: str, content: str, current_user: User
    ) -> CommentDetail:
        ...
        if not self._check_comment_permission(comment, current_user, "update"):
            raise CommentPermissionError(
                action="update", comment_id=comment_id, user_id=str(current_user.id)
            )
        ...
        # Convert to response format
        comment_detail = await self._convert_to_comment_detail(
            updated_comment, None, current_user
        )
        return comment_detail
    ...
    async def delete_comment_with_permission(
        self, comment_id: str, current_user: User
    ) -> bool:
        ...
        if not self._check_comment_permission(comment, current_user, "delete"):
            raise CommentPermissionError(
                action="delete", comment_id=comment_id, user_id=str(current_user.id)
            )
    ...
    async def _update_comment_reaction_counts(self, comment_id: str) -> None:
        ...
        try:
            ...
            like_count = await UserReaction.find(
                {"target_type": "comment", "target_id": comment_id, "liked": True}
            ).count()

            dislike_count = await UserReaction.find(
                {"target_type": "comment", "target_id": comment_id, "disliked": True}
            ).count()
    ...
    def _check_comment_permission(
        self, comment: Comment, user: User, action: str
    ) -> bool:
    ...
    async def _convert_to_comment_detail(
        ...
        if (
            metadata.get("subtype") == "service_inquiry"
            and metadata.get("isPublic") is False
        ):
