...
class AtlasMonitoringService:
    ...
    def __init__(self):
        ...
        self._auth = (
            HTTPDigestAuth(self._public_key, self._private_key)
        ...
        try:
            ...
            if clusters_response.status_code == 200:
                ...
                if clusters:
                    ...
                    logger.warning(
                        f"설정된 클러스터명 '{self._cluster_name}'을 찾을 수 없어 첫 번째 클러스터 사용: '{first_cluster.get('name')}'"
                    )
                ...
                logger.error(
                    f"클러스터 목록 조회 실패: {clusters_response.status_code}"
                )
                ...
                if response.status_code == 200:
                    data = response.json()
                    logger.info(
                        f"클러스터 정보 조회 성공: {data.get('name', 'Unknown')}"
                    )
                    return data
                else:
    ...
    def _get_cluster_metrics_sync(
        self, metric_name: str, granularity: str = "PT1M"
    ) -> List[Dict[str, Any]]:
        """클러스터 메트릭 조회 (동기)"""
        try:
            # 시간 범위 설정 (최근 5분)
    ...
    def is_configured(self) -> bool:
        """Atlas 모니터링 설정 여부 확인"""
        return all(
            [self._public_key, self._private_key, self._group_id, self._cluster_name]
        )
