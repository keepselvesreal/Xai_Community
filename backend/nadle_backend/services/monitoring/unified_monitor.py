...
class UnifiedMonitoringService:
    ...
    async def get_all_infrastructure_status(self) -> UnifiedMonitoringResponse:
        ...
        try:
            ...
            logger.info(
                f"통합 모니터링 완료: {len(infrastructure_statuses)}개 인프라, 전체 상태: {overall_status.value}"
            )
            return response
    ...
    def _calculate_overall_status(
        self, statuses: List[InfrastructureStatus]
    ) -> ServiceStatus:
        """전체 상태 계산"""
        if not statuses:
        ...
        try:
            ...
            for status in statuses:
                ...
                if status.metrics:
                    ...
                    # 응답시간
                    if (
                        hasattr(metrics, "response_time_ms")
                    ...
                    if status.infrastructure_type == InfrastructureType.CLOUD_RUN:
                        if (
                            hasattr(metrics, "cpu_utilization")
                            and metrics.cpu_utilization
                        ):
                            service_metrics["cpu_utilization"] = metrics.cpu_utilization
                        if (
                            hasattr(metrics, "memory_utilization")
                        ...
                        if (
                            hasattr(metrics, "instance_count")
                            and metrics.instance_count
                        ):
                    ...
                    # Vercel 특화 메트릭
                        ...
                        if (
                            hasattr(metrics, "deployment_status")
                            and metrics.deployment_status
                        ):
                            service_metrics["deployment_status"] = (
                                metrics.deployment_status
                            )
                        if (
                            hasattr(metrics, "core_web_vitals_score")
                            and metrics.core_web_vitals_score
                        ):
                            service_metrics["core_web_vitals_score"] = (
                    ...
                    # Upstash 특화 메트릭
                        ...
                        if (
                            hasattr(metrics, "memory_usage_percent")
                            and metrics.memory_usage_percent
                        ):
                            service_metrics["memory_usage_percent"] = (
            ...
            summary["statistics"] = {
                ...
                "healthy_services": sum(
                    1 for s in statuses if s.status == ServiceStatus.HEALTHY
                ),
                "unhealthy_services": sum(
                    1 for s in statuses if s.status == ServiceStatus.UNHEALTHY
                ),
                "degraded_services": sum(
                    1 for s in statuses if s.status == ServiceStatus.DEGRADED
                ),
                "unknown_services": sum(
                    1 for s in statuses if s.status == ServiceStatus.UNKNOWN
                ),
            }
    ...
    async def health_check(self) -> HealthCheckResponse:
        ...
        try:
            ...
            overall_status = (
                ServiceStatus.HEALTHY if all_healthy else ServiceStatus.DEGRADED
            )

            return HealthCheckResponse(
                service="unified_monitoring", status=overall_status, checks=checks
            )
