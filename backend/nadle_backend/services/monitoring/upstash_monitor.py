...
class UpstashMonitoringService:
    ...
    async def _execute_redis_command(self, command: str, *args) -> Any:
        ...
        try:
            ...
            async with self._session.post(
                self._rest_url, json=command_list
            ) as response:
    ...
    async def _get_database_stats(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            url = (
                f"{self._console_base_url}/v2/redis/database/{self._database_id}/stats"
            )
    ...
    async def _extract_console_metrics(
        self, stats: Dict[str, Any], metrics_data: Dict[str, Any]
    ):
        ...
        try:
            async with self as service:
                ...
                return {
                    ...
                    "rest_url": (
                        self._rest_url.split("@")[-1] if self._rest_url else None
                    ),  # 민감 정보 제거
    ...
    def is_configured(self) -> bool:
        ...
        return all([self._rest_url, self._rest_token])
