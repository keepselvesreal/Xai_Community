...
class VercelMonitoringService:
    ...
    async def _get_latest_deployment(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            params = {"projectId": self._project_id, "limit": 1}
    ...
    async def _get_deployment_functions(
        ...
        try:
            async with self as service:
                ...
                if deployment:
                    metrics_data.update(
                        {
                            ...
                            "deployment_created_at": self._parse_timestamp(
                    ...
                    # 배포 함수 정보
                    functions = await service._get_deployment_functions(
                        deployment.get("uid", "")
                    )
                    if functions:
                        total_invocations = 0
    ...
    def is_configured(self) -> bool:
        ...
        return all([self._api_token, self._project_id])
