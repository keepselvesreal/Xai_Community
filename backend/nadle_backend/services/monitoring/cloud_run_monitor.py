...
class CloudRunMonitoringService:
    ...
    def _get_monitoring_client(self) -> monitoring_v3.MetricServiceClient:
        ...
        if self._monitoring_client is None:
            try:
                ...
                logger.info(
                    f"Cloud Monitoring 클라이언트 초기화 완료 (프로젝트: {self._project_id})"
                )
            ...
            interval = monitoring_v3.TimeInterval(
                {
                    ...
                    "start_time": {
                        "seconds": int((now - timedelta(minutes=5)).timestamp())
                    },
                }
            )
    ...
    async def get_metrics(self) -> CloudRunMetrics:
        ...
        try:
            ...
            # 메트릭 데이터 수집 (비동기)
            loop = asyncio.get_event_loop()
            metrics_data = await loop.run_in_executor(None, self._collect_metrics)
        ...
        try:
            ...
            try:
                ...
                if request_latency is not None:
                    metrics_data["request_latency_ms"] = (
                        float(request_latency) * 1000
                    )  # seconds to ms
                    metrics_data["response_time_ms"] = metrics_data[
                        "request_latency_ms"
                    ]
            except Exception as e:
    ...
    def is_configured(self) -> bool:
        ...
        return all([self._project_id, self._service_name, self._region])
