...
class IntelligentAlertingService:
    ...
    def __init__(
        self, notification_service: NotificationService, monitoring_service: Any = None
    ):
        """
        지능형 알림 서비스 초기화
    ...
    async def evaluate_rule(self, rule: AlertRule) -> bool:
        ...
        try:
            ...
            if self.monitoring_service:
                current_value = await self.monitoring_service.get_current_metric(
            ...
            return await self.evaluate_condition(
                rule.condition, rule.threshold, current_value
            )
    ...
    async def evaluate_condition(
        self, condition: AlertCondition, threshold: AlertThreshold, current_value: float
    ) -> bool:
        """
        알림 조건 평가
    ...
    async def send_alert(
        self, rule: AlertRule, metric_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        알림 전송
        ...
        try:
            for channel in rule.channels:
                if channel == AlertChannel.EMAIL:
                    ...
                    results["discord"] = (
                        await self.notification_service.send_discord_notification(
                            message=message,
    ...
    async def aggregate_alerts(
        self, alerts: List[Dict[str, Any]], time_window_minutes: int = 5
    ) -> Dict[str, Any]:
        """
        알림 집계
    ...
    async def get_alert_history(
        self, rule_name: str, hours: int = 24
    ) -> List[Dict[str, Any]]:
        """
        알림 이력 조회
    ...
    async def is_alert_suppressed(
        self, rule: AlertRule, suppression_rule: Dict[str, Any]
    ) -> bool:
        """
        알림 억제 여부 확인
