...
class HetrixToolsClient:
    ...
    async def get_monitor_logs(
        self, monitor_id: str, days: int = 1
    ) -> List[MonitorLog]:
        """
        모니터 로그 조회
    ...
    async def create_monitor(
        self, name: str, url: str, monitor_type: str = "website", interval: int = 1
    ) -> Monitor:
        """
        새 모니터 생성
...
class HetrixMonitoringService:
    ...
    def get_monitors(self) -> List[Monitor]:
        ...
        if not self.client:
            raise ValueError(
                "서비스가 초기화되지 않았습니다. async with 구문을 사용하세요"
            )
    ...
    def create_monitor(self, name: str, url: str, interval: int = 300) -> Monitor:
        """
