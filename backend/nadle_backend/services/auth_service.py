...
class AuthService:
    ...
    async def register_user(self, user_data: UserCreate) -> User:
        ...
        existing_user = await self.user_repository.get_by_user_handle(
    ...
    async def create_access_token(self, user: User) -> str:
        ...
        Returns:
            JWT access token
        """
        payload = {"sub": str(user.id), "email": user.email}  # ObjectId를 문자열로 변환
    ...
    async def get_user_profile(self, user_id: str) -> User:
        ...
        user_cache_dict = {
            k: v for k, v in user_dict.items() if k not in sensitive_fields
        }
    ...
    async def _create_user_session(
        ...
        try:
            ...
            session_id = await session_service.create_session(
                session_data, max_concurrent_sessions=3
            )
    ...
    async def logout(
        self, access_token: str, refresh_token: str = None, user_id: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            await blacklist_service.blacklist_token(
                access_token, access_expires_at, "user_logout", user_id
            )

            # Refresh token 블랙리스트 추가 (있는 경우)
            if refresh_token:
                await blacklist_service.blacklist_token(
                    refresh_token, refresh_expires_at, "user_logout", user_id
                )
            ...
            if user_id:
                ...
                logger.info(
                    f"사용자 세션 삭제: {user_id}, 삭제된 세션 수: {deleted_sessions}"
                )
    ...
    async def logout_all_sessions(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"전체 로그아웃 성공: {user_id}, 세션: {deleted_sessions}, 블랙리스트: {blacklisted_count}"
            )
    ...
    async def verify_token_validity(self, token: str) -> bool:
        ...
        try:
            ...
            user_id = payload.get("sub")
            if user_id:
                is_user_blacklisted = await blacklist_service.is_user_blacklisted(
                    user_id
                )
    ...
    async def login_with_secure_cookies(
        self, response, user_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        """
        try:
            from nadle_backend.utils.token_security import (
                create_secure_token_response,
    ...
    async def logout_with_secure_cookies(self, request, response) -> Dict[str, Any]:
        ...
        try:
            ...
            # 현재 토큰들 추출
            access_token = token_manager.extract_token_from_cookie(
                request, "access_token"
            )
            refresh_token = token_manager.extract_token_from_cookie(
            ...
            if access_token:
                ...
                await blacklist_service.add_to_blacklist(
                    access_token, "access_token_logout"
                )
            ...
            return {"message": "로그아웃 성공", "authentication": "cookies_cleared"}

        except Exception as e:
            logger.error(f"보안 쿠키 로그아웃 실패: {e}")
            return {"message": "로그아웃 중 오류 발생", "error": str(e)}

    async def refresh_token_with_secure_cookies(
        ...
        try:
            ...
            old_access_token = token_manager.extract_token_from_cookie(
                request, "access_token"
            )
            ...
            return {"message": "토큰 갱신 성공", "authentication": "secure_cookies"}
