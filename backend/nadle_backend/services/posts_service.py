...
class PostsService:
        ...
        if post_type in inquiry_types:
            try:
                ...
                if "content" not in content_data:
                    raise ValueError(
                        f"Missing required 'content' field for {post_type}"
                    )

                # 등록 문의는 contact, website_url 필드가 추가로 필요
                if "contact" not in content_data:
                    raise ValueError(
                        f"Missing required 'contact' field for {post_type}"
                    )
                if "website_url" not in content_data:
                    raise ValueError(
                        f"Missing required 'website_url' field for {post_type}"
                    )
    ...
    async def create_post(
        self, post_data: PostCreate, current_user: Optional[User] = None
    ) -> Post:
    ...
    async def get_post(
        self, slug_or_id: str, current_user: Optional[User] = None
    ) -> Post:
        ...
        if cached_post:
            ...
            try:
                ...
                post = Post(
                    ...
                    updated_at=(
                        datetime.fromisoformat(cached_post["updated_at"])
                        if cached_post.get("updated_at")
                        else None
                    ),
                    published_at=(
                        datetime.fromisoformat(cached_post["published_at"])
                        if cached_post.get("published_at")
                        else None
                    ),
                )
    ...
    async def list_posts(
        ...
        # 디버깅을 위한 로그 추가
        print(
            f"📋 List posts request - metadata_type: '{metadata_type}', page: {page}, page_size: {page_size}"
        )

        # 🚀 단일 aggregation 쿼리로 모든 데이터 조회
        posts_data, total = await self.post_repository.list_posts_optimized(
            page=page, page_size=page_size, metadata_type=metadata_type, sort_by=sort_by
        )

        print(
            f"📊 List posts results - found {total} posts, returned {len(posts_data)} items"
        )
        ...
        for post_data in posts_data:
            ...
            post_dict = {
                ...
                "updated_at": (
                    post_data["updated_at"].isoformat()
            ...
            if metadata_type == "moving services":
                try:
                    comment_stats = (
                        await self.comment_repository.get_comment_stats_by_post(
                            str(post_data["_id"])
                        )
                    )
                    ...
                    print(
                        f"📊 Service stats for {post_data.get('title')}: {post_dict['service_stats']}"
                    )
                except Exception as e:
    ...
    async def update_post(
        self, slug: str, update_data: PostUpdate, current_user: User
    ) -> Post:
        """Update post.
    ...
    async def search_posts(
        ...
        # 디버깅을 위한 로그 추가
        print(
            f"🔍 Search request - query: '{query}', metadata_type: '{metadata_type}', service_type: '{service_type}'"
        )
    ...
    async def toggle_post_reaction(
        ...
        if not user_reaction:
            ...
            raw_page_type = (
                getattr(post.metadata, "type", "board") if post.metadata else "board"
            )
            ...
            user_reaction = UserReaction(
                ...
                target_type="post",
                target_id=post_id,
                metadata={"route_path": route_path, "target_title": post.title},
            )

        # Toggle reaction with proper mutual exclusivity rules:
        # 1. Like/Dislike are mutually exclusive (only these two)
        # 2. Bookmark operates independently from Like/Dislike
        if reaction_type == "like":
            # Toggle like, clear dislike if setting like (bookmark remains unchanged)
            was_liked = user_reaction.liked
            user_reaction.liked = not was_liked
            if user_reaction.liked:
                user_reaction.disliked = False  # Only clear dislike, keep bookmark
        elif reaction_type == "dislike":
            # Toggle dislike, clear like if setting dislike (bookmark remains unchanged)
            was_disliked = user_reaction.disliked
            user_reaction.disliked = not was_disliked
            if user_reaction.disliked:
                user_reaction.liked = False  # Only clear like, keep bookmark
        elif reaction_type == "bookmark":
            # Toggle bookmark independently (like/dislike remain unchanged)
    ...
    async def get_service_post_with_extended_stats(
        ...
        return await self.get_service_post_with_extended_stats_from_post(
            post, current_user
        )

    async def get_service_post_with_extended_stats_from_post(
        self, post: Post, current_user: Optional[User] = None
    ) -> Dict[str, Any]:
    ...
    async def list_service_posts_with_extended_stats(
        ...
        """
        print(
            f"📋 List service posts with extended stats - page: {page}, page_size: {page_size}"
        )
        ...
        for post_dict in result["items"]:
            try:
                ...
                # 댓글 통계 조회
                comment_stats = await comment_repository.get_comment_stats_by_post(
                    str(post_id)
                )
                ...
                enhanced_items.append(post_dict)
                print(
                    f"✅ Enhanced post {post_id} with extended stats: {comment_stats}"
                )
    ...
    async def get_user_reaction_cached(
        ...
        try:
            ...
            if reaction:
                ...
                reaction_info = {"liked": False, "disliked": False, "bookmarked": False}
            ...
            return {"liked": False, "disliked": False, "bookmarked": False}
    ...
    async def get_authors_info_batch(
        ...
        if uncached_ids:
            try:
                ...
                if object_ids:
                    ...
                    print(
                        f"🔄 배치 조회: {len(object_ids)}개 요청 → {len(authors)}개 결과"
                    )
    ...
    async def get_user_reactions_batch(
        ...
        if uncached_post_ids:
            try:
                ...
                print(
                    f"🔄 사용자 반응 배치 조회: {len(uncached_post_ids)}개 요청 → {len(reactions)}개 결과"
                )
    ...
    async def get_comments_with_batch_authors(
        self, post_slug: str
    ) -> List[Dict[str, Any]]:
        ...
        def add_author_info_recursive(item):
            ...
            comment_dict = {
                ...
                "replies": [
                    add_author_info_recursive(reply_item) for reply_item in replies
                ],
            }
            return comment_dict
        ...
        print(
            f"📊 배치 조회로 {len(all_comments)}개 댓글에 {len(authors_info)}명의 작성자 정보 결합 완료"
        )
    ...
    async def get_post_with_author_aggregated(
        self, post_slug: str
        ...
        try:
            ...
            if results:
                result = results[0]
                print(
                    f"🔄 Aggregation으로 게시글 + 작성자 정보 한 번에 조회 완료 - {post_slug}"
                )
                return result
            else:
    ...
    async def get_post_with_comments_aggregated(
        self, post_slug: str
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(
                f"🔄 Aggregation + 배치 조회로 게시글 + 댓글 + 작성자 정보 조회 완료 - {post_slug}"
            )
    ...
    async def get_post_with_everything_aggregated(
        self, post_slug: str, current_user_id: Optional[str] = None
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if current_user_id:
                addfields_stage["$addFields"]["user_reaction"] = {
                    "$let": {
                        "vars": {
                            "reaction": {"$arrayElemAt": ["$user_reaction_raw", 0]}
                        },
                        "in": {
                            "$cond": {
                                "if": {"$ne": ["$$reaction", None]},
            ...
            if results:
                result = results[0]
                print(
                    f"✅ 완전 통합 Aggregation으로 모든 데이터 한 번에 조회 완료 - {post_slug}"
                )
                print(f"📊 조회된 댓글 수: {len(result.get('comments', []))}")
                print(
                    f"👤 게시글 작성자: {result.get('author', {}).get('user_handle', 'N/A')}"
                )
