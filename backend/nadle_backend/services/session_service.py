...
class SessionService:
    ...
    async def create_session(
        ...
        try:
            ...
            # 사용자 세션 목록 업데이트
            await redis_manager.set(user_sessions_key, user_sessions, ttl=ttl)

            logger.info(
                f"세션 생성 성공: {session_id} (사용자: {session_data.user_id})"
            )
            return session_id
    ...
    async def delete_user_sessions(self, user_id: str) -> int:
        ...
        try:
            ...
            logger.info(
                f"사용자 세션 삭제 완료: {user_id}, 삭제된 세션 수: {deleted_count}"
            )
    ...
    async def get_active_sessions_count(self) -> int:
        """활성 세션 개수 조회 (일일 활성 사용자 계산용)"""
        redis_manager = await get_redis_manager()

        if not await redis_manager.is_connected():
            logger.warning("Redis 연결 없음 - 기본값 반환")
            return 0

        try:
            # 세션 키 패턴으로 검색
            session_pattern = get_prefixed_key(f"{self.session_prefix}*")
            session_keys = await redis_manager.get_keys_by_pattern(session_pattern)
            ...
            for session_key in session_keys:
                try:
                    ...
                    logger.debug(f"세션 데이터 파싱 오류 (스킵): {e}")
                    continue

            return active_count

        except Exception as e:
            logger.error(f"활성 세션 개수 조회 오류: {e}")
            return 0
