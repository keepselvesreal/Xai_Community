...
class UserActivityService:
    ...
    async def _get_user_reactions_grouped(
        ...
        page_grouped_reactions = await self._get_user_reactions_grouped_paginated(
            user_id, limit=1000, skip=0
        )

        # Flatten page grouping for backward compatibility
        result = {"likes": [], "bookmarks": [], "dislikes": []}
        ...
        for reaction_type in ["likes", "bookmarks", "dislikes"]:
            ...
            for page_type in [
                ...
                result[reaction_type].extend(
                    page_grouped_reactions[reaction_key][page_type]
                )
        ...
        for i, reaction in enumerate(reactions):
            ...
            if reaction.target_type == "post" and reaction.target_id:
                try:
                    ...
                    if post.status == "deleted":
                        logger.info(
                            f"❌ SKIPPING reaction {reaction.id} - post is deleted"
                        )
                        continue

                    # 실제 게시글의 현재 정보를 사용하여 페이지 타입과 route_path 생성
                    raw_page_type = (
                        getattr(post.metadata, "type", "board")
                        if post.metadata
                        else "board"
                    )
