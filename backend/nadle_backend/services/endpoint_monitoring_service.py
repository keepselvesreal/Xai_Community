...
class EndpointMonitoringService:
    ...
    async def check_all_endpoints(self) -> EndpointMonitoringResult:
        ...
        try:
            ...
            for endpoint_config in self.endpoints:
                task = asyncio.create_task(self._check_endpoint(endpoint_config))
                tasks.append(task)
            ...
            logger.info(
                f"API 엔드포인트 체크 완료: {result.healthy_count}/{result.total_endpoints} 정상"
            )
            return result
    ...
    def _determine_endpoint_status(
        self, status_code: int, response_time: float, expected_statuses: List[int]
    ) -> str:
    ...
    def _aggregate_results(
        ...
        # 평균 응답 시간 계산
        if endpoints:
            average_response_time = sum(ep.response_time for ep in endpoints) / len(
                endpoints
            )
        else:
            average_response_time = 0.0
    ...
    async def check_specific_endpoint(
        self, endpoint_name: str
    ) -> Optional[EndpointStatus]:
        """특정 엔드포인트만 체크합니다."""
        try:
            endpoint_config = next(
                (ep for ep in self.endpoints if ep["name"] == endpoint_name), None
            )
    ...
    async def get_endpoint_history(
        self, endpoint_name: str, hours: int = 24
    ) -> List[Dict[str, Any]]:
        """
