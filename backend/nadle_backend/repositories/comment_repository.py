...
class CommentRepository:
    ...
    async def create(
        self, comment_data: CommentCreate, author_id: str, parent_id: str
    ) -> Comment:
        ...
        if comment_data.parent_comment_id:
            ...
            await self._validate_reply_depth(
                comment_data.parent_comment_id, settings.max_comment_depth
            )
    ...
    async def list_by_post(
        ...
        sort_field = (
            f"-{sort_by}" if sort_by in ["created_at", "updated_at"] else sort_by
        )
    ...
    async def get_replies(
        self, parent_comment_id: str, status: str = "active"
    ) -> List[Comment]:
        ...
        """
        query = {"parent_comment_id": parent_comment_id, "status": status}

        print(
            f"🔍 [DEBUG] get_replies 쿼리: parent_comment_id={parent_comment_id}, status={status}"
        )
        replies = await Comment.find(query).sort("created_at").to_list()
        print(f"🔍 [DEBUG] get_replies 결과: {len(replies)}개 답글 발견")
        for reply in replies:
            print(
                f"    답글: id={reply.id}, content={reply.content[:30]}..., parent_comment_id={reply.parent_comment_id}"
            )
    ...
    async def get_replies_recursive(
        ...
        for reply in replies:
            ...
            nested_replies = await self.get_replies_recursive(
                str(reply.id), status, max_depth, current_depth + 1
            )

            reply_dict = {"comment": reply, "replies": nested_replies}
    ...
    async def get_comments_with_replies(
        ...
        top_comments, total = await self.list_by_post(
            post_id=post_id, page=page, page_size=page_size, status=status
        )
        ...
        for comment in top_comments:
            ...
            replies = await self.get_replies_recursive(
                str(comment.id), status=status, max_depth=max_depth
            )

            comment_dict = {"comment": comment, "replies": replies}
    ...
    async def count_by_post(self, post_id: str, status: str = "active") -> int:
        ...
        Returns:
            Total comment count
        """
        query = {"parent_id": post_id, "status": status}
    ...
    async def _validate_reply_depth(
        self, parent_comment_id: str, max_depth: int = 3
    ) -> None:
    ...
    async def count_comments_by_subtype(
        self, post_id: str, subtype: CommentSubtype
    ) -> int:
        ...
        try:
            ...
            print(
                f"Error counting comments by subtype {subtype} for post {post_id}: {e}"
            )
            return 0
    ...
    async def get_comment_stats_by_post(self, post_id: str) -> Dict[str, int]:
        ...
        try:
            ...
            pipeline = [
                {"$match": {"parent_id": post_id, "status": "active"}},
