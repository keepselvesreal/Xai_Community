...
class UserReactionRepository:
    ...
    async def find_by_user(self, user_id: str) -> List[UserReaction]:
        ...
        try:
            reactions = (
                await UserReaction.find({"user_id": user_id})
                .sort("-created_at")
                .to_list()
            )
            return reactions
    ...
    async def count_reactions_by_type(self, target_type: str, target_id: str) -> dict:
        ...
        try:
            like_count = await UserReaction.find(
                {"target_type": target_type, "target_id": target_id, "liked": True}
            ).count()

            dislike_count = await UserReaction.find(
                {"target_type": target_type, "target_id": target_id, "disliked": True}
            ).count()

            bookmark_count = await UserReaction.find(
                {"target_type": target_type, "target_id": target_id, "bookmarked": True}
            ).count()
            ...
            return {"like_count": 0, "dislike_count": 0, "bookmark_count": 0}
