...
def get_alerting_service(
    ...
    if _global_alerting_service is None:
        _global_alerting_service = IntelligentAlertingService(
...
async def create_alert_rule(
    ...
    try:
        ...
        # 중복 이름 체크
        existing_rule = await alerting_service.rule_engine.get_rule_by_name(
            alert_rule.name
        )
        if existing_rule:
            raise HTTPException(
...
async def update_alert_rule(
    ...
    try:
        ...
        updated_rule = AlertRule(
            ...
            condition=(
                AlertCondition(rule_data.condition)
                if rule_data.condition
                else existing_rule.condition
            ),
            threshold=AlertThreshold(
                metric=(
                    rule_data.threshold.metric
                    if rule_data.threshold
                    else existing_rule.threshold.metric
                ),
                value=(
                    rule_data.threshold.value
                    if rule_data.threshold
                    else existing_rule.threshold.value
                ),
                duration_minutes=(
                    rule_data.threshold.duration_minutes
                    if rule_data.threshold
                    else existing_rule.threshold.duration_minutes
                ),
            ),
            severity=(
                AlertSeverity(rule_data.severity)
                if rule_data.severity
                else existing_rule.severity
            ),
            channels=(
                [AlertChannel(channel) for channel in rule_data.channels]
                if rule_data.channels
                else existing_rule.channels
            ),
            cooldown_minutes=(
                rule_data.cooldown_minutes
                if rule_data.cooldown_minutes is not None
                else existing_rule.cooldown_minutes
            ),
            escalation_minutes=(
                rule_data.escalation_minutes
                if rule_data.escalation_minutes is not None
                else existing_rule.escalation_minutes
            ),
            enabled=(
                rule_data.enabled
...
async def send_manual_alert(
    ...
    try:
        ...
        logger.error(
            f"수동 알림 전송 실패 ({alert_data.get('rule_id', 'unknown')}): {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    try:
        ...
        for severity in AlertSeverity:
            severity_stats[severity.value] = len(
                [rule for rule in active_rules if rule.severity == severity]
            )

        # 채널별 통계
        channel_stats = {}
        for channel in AlertChannel:
            channel_stats[channel.value] = len(
                [rule for rule in active_rules if channel in rule.channels]
            )
        ...
        return AlertStatisticsResponse(
            ...
            alerts_sent_today=len(
                alerting_service.alert_history
            ),  # 임시로 같은 값 사용
...
async def health_check(
    ...
    try:
        ...
        active_rules = len(
            [
                rule
                for rule in alerting_service.rule_engine.rules.values()
                if rule.enabled
            ]
        )
...
async def test_alert_system(
    ...
    try:
        ...
        test_rule = AlertRule(
            ...
            threshold=AlertThreshold(
                metric="test_metric", value=100.0, duration_minutes=1
            ),
        ...
        return {
            "test_status": "failed",
            "error": str(e),
            "system_health": {"rule_engine": "error", "notification_service": "error"},
            "test_timestamp": datetime.utcnow().isoformat(),
        }
...
async def create_suppression_rule(
    ...
    try:
        ...
        suppression_rule = {
            ...
            "expires_at": (
                suppression_data.expires_at.isoformat()
                if suppression_data.expires_at
                else None
            ),
        }
...
async def get_suppression_rules(
    ...
    try:
        ...
        return {"suppression_rules": suppression_rules}
