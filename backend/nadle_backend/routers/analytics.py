...
async def simulate_user_activity(
    activity_type: str = Query(..., description="시뮬레이션할 활동 타입"),
    count: int = Query(default=1, le=10, description="생성할 활동 수"),
    analytics_service: AnalyticsService = Depends(get_analytics_service),
    current_user: User = Depends(get_current_user),
):
    """
    ...
    try:
        ...
        result = await analytics_service.simulate_user_activity(activity_type, count)
        return {
            "message": f"{activity_type} 활동 {count}건이 생성되었습니다.",
            "details": result,
        }

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"활동 시뮬레이션 중 오류가 발생했습니다: {str(e)}"
        )
...
async def reset_test_data(
    ...
    try:
        ...
        result = await analytics_service.reset_test_data()
        return {"message": "테스트 데이터가 초기화되었습니다.", "details": result}

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"데이터 초기화 중 오류가 발생했습니다: {str(e)}"
        )
