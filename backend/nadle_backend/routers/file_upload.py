...
from nadle_backend.services.file_storage import generate_file_path, save_file_to_disk
...
async def upload_file(
    ...
    try:
        logger.info(
            f"File upload request received: {file.filename}, attachment_type: {attachment_type}"
        )
        ...
        file_document = create_file_document(
            metadata, attachment_type=attachment_type, attachment_id=attachment_id
        )

        # Save to database
        logger.info(
            f"Attempting to save file record to database: {file_document['file_id']}"
        )
        ...
        return {
            ...
            "stored_filename": file_document.get(
                "stored_filename", file_document["file_id"]
            ),
