...
class UpstashRedisManager:
    ...
    async def connect(self) -> bool:
        ...
        if not self.rest_url or not self.rest_token:
            logger.warning(
                "Upstash Redis 설정이 없습니다. (UPSTASH_REDIS_REST_URL, UPSTASH_REDIS_REST_TOKEN)"
            )
            return False
    ...
    async def _request(self, command: list) -> Dict[str, Any]:
        ...
        if not self.session:
            raise RuntimeError(
                "Upstash Redis 세션이 초기화되지 않았습니다. connect()를 먼저 호출하세요."
            )
        ...
        try:
            if await self.is_connected():
                ...
                return {
                    ...
                    "total_commands_processed": info_dict.get(
