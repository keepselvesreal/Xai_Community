...
# Cloud Run 배포 스크립트
# 사용법: ./deploy.sh [PROJECT_ID] [REGION] [SERVICE_NAME]
...
SERVICE_NAME=${3:-"cloud-run-test"}
IMAGE_NAME="gcr.io/${PROJECT_ID}/${SERVICE_NAME}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

log_info "========================================="
log_info "    Cloud Run 배포 스크립트 시작"
log_info "========================================="

# 프로젝트 ID 확인
if [ -z "$PROJECT_ID" ]; then
    log_error "프로젝트 ID가 필요합니다."
    echo "사용법: $0 [PROJECT_ID] [REGION] [SERVICE_NAME]"
    echo "예시: $0 my-project-id asia-northeast3 cloud-run-test"
    exit 1
fi
