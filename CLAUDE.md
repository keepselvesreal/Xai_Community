...
- 서버 실행이 필요한 경우 이미 동작 중인 서버가 있는지 확인하고, 동작 중인 서버를 우선적으로 사용한다.
- 사용자가 제공한 정보도 비판적으로 검토하며, 무조건 신뢰하지 않는다.
- 패키지 설치 시 가상환경 등이 있는지 반드시 확인한다. 
- 추측이나 가정 대신 실제 코드나 파일에 기반해 작업한다. 

### Coding
- 폴더나 파일을 만들 때는 기존 프로젝트 구조의 체계를 반영한다.
- 타입 검사를 활용하여 런타임 이전에 감지 가능한 오류를 사전에 방지한다.
- 오류 발생 가능성이 있는 부분에는 디버깅을 위한 코드 또는 로깅을 적절히 삽입한다.
- 모든 코드 파일의 맨 위에는 아래 요소를 포함하는 '목차'를 작성한다.
    - 작업 시간(date 명령어로 직접 확인한 현재 한국 시간 기준)
    - 작업 버전
    - 주요 컴포넌트들
    - 주요 컴포넌트를 구성하는 함수 이름과 함수의 핵심 내용
    - 코드 라인 정보
- 관련된 파일, 함수 등이 있는 경우 이 관계(들)도 목차에 정리한다.  
- 모든 코드 파일의 목차는 파일 내용이 바뀔 때마다 반드시 최신화한다.
- 기존 파일을 다룰 때는 반드시 상단의 목차를 참고해 작업을 진행한다.

### 메모리

- **프론트엔드 포트 정보**: 현재 프런트엔드 서버가 사용하는 포트는 3000이 아니라 5173입니다.